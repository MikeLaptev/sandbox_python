...
def loadTestSuitesWithAllTestByDiscover():
    ...
    list_of_modules = example_of_loader.discover(
        ".", "generate_and_load_unittest_update_[a-z][a-z][a-z].py"
    )
    ...
    for module in list_of_modules:
        ...
        print(
            (
                "No errors occur "
                if len(actual_result.errors) == 0
                else "{} error(s) occurs".format(len(actual_result.errors))
            )
        )
        print(
            (
                "No skipped tests "
                if len(actual_result.skipped) == 0
                else "{} test(s) has been skipped".format(len(actual_result.skipped))
            )
        )
        print(
            (
                "Run was successful "
                if actual_result.wasSuccessful()
                else "Run contains test that did not complete successfully"
            )
        )
...
def loadTestSuitesWithSelectedTestsByDiscover():
    ...
    example_of_loader = unittest.TestLoader()
    list_of_modules = example_of_loader.discover(
        ".", "generate_and_load_unittest_update_[a-z][a-z][a-z][a-z].py"
    )
    # Each module has type unittest.TestSuite
    # Lets run all these modules one by one
    for module in list_of_modules:
