...
class MyOwnJSONProcessing:
    ...
    @classmethod
    def generate_set_of_files_with_json_obj(
        cls, amount_of_files, is_data_complicated=False
    ):
        for dummy_i in range(amount_of_files):
            if not is_data_complicated:
                ...
                cls.generate_json_file_with_data(
                    data=cls.generate_complicated_data_for_json_obj()
                )
    ...
    def generate_data_for_json_obj(cls):
        ...
        for dummy_i in range(
            random.randrange(cls.min_len_of_json_dict, cls.max_len_of_json_dict)
        ):
            new_key = cls.randomword(
                random.randrange(cls.min_len_of_key, cls.max_len_of_key)
            )
            new_value = random.randrange(cls.max_value)
            if new_key not in json_data:
    ...
    @staticmethod
    def generate_json_file_with_data(
        file_name_template="data_<timestamp>.json", data={}
    ):
        """
        ...
        file_name_id = 0
        file_name = (
            string.replace(file_name_template, "<timestamp>", str(time.time()))
            if (string.find(file_name_template, "<timestamp>") != -1)
            else file_name_template
        )
        while os.path.exists(file_name):
            ...
            file_name = (
                string.replace(file_name_template, "<timestamp>", str(time.time()))
                if (string.find(file_name_template, "<timestamp>") != -1)
                else string.replace(file_name_template, ".", str(file_name_id) + ".")
            )
        # process the file
