...
def get_list_of_tests_by_filter(dir_with_tests, file_name_filter):
    ...
    for full_file_name in all_files:
        search_results = re.search(
            re.escape(os.sep) + file_name_filter + "$", full_file_name
        )
...
def run_test_module(module_name):
    subprocess.call(
        [python_exe_name, unittest_launch_parameters, unittest_module_name, module_name]
    )
...
if __name__ == "__main__":
    ...
    for full_file_name in list_of_test_files:
        ...
        threads.append(
            threading.Thread(target=run_test_module, args=(search_results.group(1),))
        )
