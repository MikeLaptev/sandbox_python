...
class CData_Preparation(object):
    ...
    def __str__(self):
        string_to_return = "String variable: {}{}".format(self.string_variable, os.linesep)
        string_to_return += "Number variable: {}{}".format(self.number_variable, os.linesep)
        string_to_return += "Dictionary: {}".format(os.linesep)
        for dict_key, dict_value in self.dict_variable.items():
            string_to_return += "Key: {} => Value: {}{}".format(dict_key, dict_value, os.linesep)
...
def generate_deep_dict(amount_of_levels = 1, max_amount_of_leaves = 5, min_len_of_obj_in_dict = 2, max_len_of_obj_in_dict = 5, min_len_of_key = 1, max_len_of_key = 10):
    ...
    if amount_of_levels == 1:
        for dummy_i in range(random.randrange(1, max_amount_of_leaves)):
            ...
            if object_type == 1:
                # it should be dict
                value_to_add = generate_simple_dict(min_len_of_obj_in_dict, max_len_of_obj_in_dict, min_len_of_key, max_len_of_key)
            elif object_type == 2:
                # it should be array
                value_to_add = generate_simple_array(min_len_of_obj_in_dict, max_len_of_obj_in_dict, min_len_of_key, max_len_of_key)
            elif object_type == 3:
                # it should be string
...
def generate_simple_dict(min_len_of_obj_in_dict = 1, max_len_of_obj_in_dict = 5, min_len_of_key = 1, max_len_of_key = 10):
    data = dict()
    # generating random key
    for dummy_i in range(random.randrange(min_len_of_obj_in_dict, max_len_of_obj_in_dict)):
        ...
        if not data.has_key(new_key):
            data[new_key] = new_value
            
    return data

def generate_simple_array(min_len_of_obj_in_dict = 1, max_len_of_obj_in_dict = 5, min_len_of_key = 1, max_len_of_key = 10):
    data = list()
    # generating random key
    for dummy_i in range(random.randrange(min_len_of_obj_in_dict, max_len_of_obj_in_dict)):
        new_value = randomword(random.randrange(min_len_of_key, max_len_of_key))
