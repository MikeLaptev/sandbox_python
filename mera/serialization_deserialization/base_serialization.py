...
class Base_Serialization(object):
    ...
    def deserialize_me(self):
        ...
        deserialized_object = self.deserialize_object(
            self.data_object.__class__.__name__
        )
        if self.data_object != deserialized_object:
            print(
                (
                    "Incorrect serialization/deserialization of Data object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.data_object, deserialized_object
                    )
                )
            )

        # Array
        deserialized_array = self.deserialize_object(
            self.array_object.__class__.__name__
        )
        if self.array_object != deserialized_array:
            print(
                (
                    "Incorrect serialization/deserialization of Array object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.array_object, deserialized_array
                    )
                )
            )

        # Dictionary
        deserialized_dictionary = self.deserialize_object(
            self.dict_object.__class__.__name__
        )
        if self.dict_object != deserialized_dictionary:
            print(
                (
                    "Incorrect serialization/deserialization of Dict object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.dict_object, deserialized_dictionary
                    )
                )
            )
        ...
        if self.set_object != deserialized_set:
            print(
                (
                    "Incorrect serialization/deserialization of Set object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.set_object, deserialized_set
                    )
                )
            )

        # Tuple
        deserialized_tuple = self.deserialize_object(
            self.tuple_object.__class__.__name__
        )
        if self.tuple_object != deserialized_tuple:
            print(
                (
                    "Incorrect serialization/deserialization of Tuple object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.tuple_object, deserialized_tuple
                    )
                )
            )

        # String
        deserialized_string = self.deserialize_object(
            self.string_object.__class__.__name__
        )
        if self.string_object != deserialized_string:
            print(
                (
                    "Incorrect serialization/deserialization of String object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.string_object, deserialized_string
                    )
                )
            )

        # Integer
        deserialized_integer = self.deserialize_object(
            self.int_object.__class__.__name__
        )
        if self.int_object != deserialized_integer:
            print(
                (
                    "Incorrect serialization/deserialization of Number object. {0}Expected: {1}{0}Actual: {2}".format(
                        os.linesep, self.int_object, deserialized_integer
                    )
                )
            )
