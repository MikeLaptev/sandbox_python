...
class MyOwnJSONProcessing:
    ...
    def generate_data_for_json_obj(self):
        ...
        for dummy_i in range(
            random.randrange(self.min_len_of_json_dict, self.max_len_of_json_dict)
        ):
            new_key = self.randomword(
                random.randrange(self.min_len_of_key, self.max_len_of_key)
            )
    ...
    def generate_json_file_with_data(
        self, file_name_template="data_<timestamp>.json", data={}
    ):
        """
