...
# Project's imports
from mera.selenium_training_automation.model.mailbox_configuration import NewMailBoxSettings
...
@pytest.mark.parametrize(argnames="settings",
                         argvalues=[
                             NewMailBoxSettings(),
...
def test_create_mailbox(settings, application):
    ...
    assert application.get_title() == expected_title, \
        "Incorrect title on the registration page. Step 2. Expected: {}. Actual {}".format(
            expected_title,
            application.get_title())

    # check header
    expected_header = 'Sign up'
    assert application.registration_page.header.text == expected_header, \
        "Incorrect header on the registration page. Step 3. Expected: {}. Actual {}".format(
            expected_header,
            application.registration_page.header.text)
    ...
    # birthday
    application.registration_page.month.select_by_visible_text(settings.user_birth_month)
    ...
    if application.registration_page.is_validation_message_box_visible:
        ...
        assert not application.registration_page.is_name_validation_message_visible, \
            application.registration_page.name_validation_message.text
        assert not application.registration_page.is_username_validation_message_visible, \
            application.registration_page.username_validation_message.text
        assert not application.registration_page.is_password_validation_message_visible, \
            application.registration_page.password_validation_message.text
        assert not application.registration_page.is_mobile_validation_message_visible, \
            application.registration_page.mobile_validation_message.text
        assert not application.registration_page.is_day_of_birth_validation_message_visible, \
            application.registration_page.day_of_birth_validation_message.text
        assert not application.registration_page.is_gender_validation_message_visible, \
    ...
    assert application.verification_page.header.text == expected_header, \
        "Incorrect header on the verification page. Step 5. Expected: {}. Actual {}".format(
            expected_header,
            application.verification_page.header.text)

    # check mobile
    assert application.verification_page.mobile_value == settings.user_phone_number, \
        "Incorrect mobile has been displayed on the verification page. Step 6. Expected: {}. Actual {}".format(
            settings.user_phone_number,
            application.verification_page.mobile_value)

    # check country-code
    assert application.verification_page.country_code == "+" + settings.user_country_code, \
        "Incorrect mobile has been displayed on the verification page. Step 6. Expected: +{}. Actual {}".format(
            settings.user_country_code,
            application.verification_page.country_code)
