...
def difference_between_vectors(minuend_vector, subtrahend_vector):
    ...
    [1, 2, 3]
    """
    return [minuend_vector[i] - subtrahend_vector[i] for i in range(len(minuend_vector))]
...
def process_initial_data(list_of_vectors, result_vector):
    ...
    for equation_id in range(len(list_of_vectors)):
        ...
        processed_vector.append(scalar_multiplication(result_vector, list_of_vectors[equation_id]))
        # calculate value for equation
        processed_matrix.append([scalar_multiplication(list_of_vectors[equation_id], list_of_vectors[another_equation_id]) for another_equation_id in range(len(list_of_vectors))])
...
def ordinary_least_squares(list_of_vectors, result_vector):
    ...
    [6.0, 2.5]
    """
    processed_matrix, processed_vector = process_initial_data(list_of_vectors, result_vector)
...
def calculate_minimum_of_discrepancy(list_of_vectors, result_vector):
    ...
    for position in range(len(list_of_vectors)):
        difference = difference_between_vectors(difference,
                                                multiply_vector_by_value(list_of_vectors[position],
                                                                         discrepancy_vector[position]))
