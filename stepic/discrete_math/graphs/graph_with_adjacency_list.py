...
class Graph(object):
    ...
    def vertexes_names(self):
        return list(self._vertexes.keys())

    @property
    def vertexes(self):
        return list(self._vertexes.values())
    ...
    def adjacency_list_for_vertex(self, vertex):
        ...
        if vertex_object is not None:
            return [
                self._vertexes.get(_id)
                for _id in vertex_object.adjacency_list_of_vertex_ids
            ]
        return None

    def __str__(self):
        output_string = ""
        output_string += "The graphs contains {} vertexes {}".format(
            len(self._vertexes), linesep
        )
        output_string += "Edges are: {}".format(linesep)
        for vertex_name, vertex_object in list(self._vertexes.items()):
            output_string += "From {} to {}{}".format(
                vertex_object.name,
                list(vertex_object.adjacency_list_of_vertex_ids),
                linesep,
            )
