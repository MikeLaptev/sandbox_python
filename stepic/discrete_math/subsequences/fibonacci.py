...
def calculate_by_position(positions):
    ...
    return (((1+square_root_of_five)/2)**positions - ((1-square_root_of_five)/2)**positions)/square_root_of_five
...
if __name__ == "__main__":
    ...
    multi_thread_result = p.map_async(calculate_by_position,
                                      range(amount_of_numbers),
                                      amount_of_numbers/number_of_threads)
    for position_of_number, number in enumerate(generate_value_fibonacci(amount_of_numbers)):
        element_from_multithread_solution = multi_thread_result.get()[position_of_number]
        if (element_from_multithread_solution - number) > 1e-8:
            # measurement error on 36 iteration :(
            print("Number {} at {} position should be {}".format(
    ...
    # Time measurements
    sequential_launch = \
        timeit('[a for a in generate_value_fibonacci(amount_of_numbers)]',
    ...
    parallel_launch = \
        timeit('p.map_async(calculate_by_position, range(amount_of_numbers), amount_of_numbers/number_of_threads).ready()',
